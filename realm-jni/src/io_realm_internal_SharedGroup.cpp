...
class RowArray {
public:
    RowArray(JNIEnv* env, jlongArray arr)
    : m_env(env)
    , m_array(arr)
    , m_ptr(env->GetLongArrayElements(m_array, NULL))
    {
    }

    ~RowArray() {
        if (m_ptr)
            m_env->ReleaseLongArrayElements(m_array, m_ptr, JNI_ABORT);
    }
    ...
    Row const& operator[](size_t i) const {
        return reinterpret_cast<const Row*>(m_ptr)[i];
    }
...
(JNIEnv *env, jobject, jlong native_ptr, jlongArray observedRows)
{
    TR_ENTER_PTR(native_ptr)
    RowArray rows(env, observedRows);
    try {
        ModifiedRowParser m;
        SG(native_ptr)->advance_read(m);
